...
#ifndef MP_RDMA_SOCKET_H
#define MP_RDMA_SOCKET_H
 ...
 * \ingroup socket
 *
 * \brief (abstract) base class of all MpRDMASockets
 *
 * This class exists solely for hosting MpRDMASocket attributes that can
 * be reused across different implementations.
 */
class MpRDMASocket : public Socket
{
public:
  /**
   * Get the type ID.
  ...
  MpRDMASocket (void);
  virtual ~MpRDMASocket (void);

  /**
   * \ingroup mp-rdma
   * \brief Names of the 11 MpRDMA states 
   * \note MP_RDMA won't use so many states, we just reuse TCP states to reuse some TCP code 
   */
  typedef enum {
  ...
  /**
   * \brief Literal names of MpRDMA states for use in log messages
   */
  static const char* const MpRDMAStateName[MpRDMASocket::LAST_STATE]; 
...
/**
 * \ingroup mp-rdma
 * TracedValue Callback signature for MpRDMAStates_t
 *
 * \param [in] oldValue original value of the traced variable
 * \param [in] newValue new value of the traced variable
 */
typedef void (* MpRDMAStatesTracedValueCallback)(const MpRDMASocket::MpRDMAStates_t oldValue,
                                              const MpRDMASocket::MpRDMAStates_t newValue);

} // namespace ns3

#endif /* MpRDMA_SOCKET_H */ 
