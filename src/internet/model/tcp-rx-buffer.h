...
public:
  ...
  bool Add (Ptr<Packet> p, MpRDMAHeader const& mpRDMAh); 
  ...
  SequenceNumber32 MaxSeqInBuf() {return m_maxSeqInBuf;}
  void SetSndL(uint32_t sndL) {m_sndL = sndL;}
  void SetRcvL(uint32_t rcvL) {m_rcvL = rcvL;}
...
private:
  ...
  /*====================== for out-of-order measurement ====================== */
  SequenceNumber32 m_maxSeqInBuf;  //current maximum sequence in buffer 
  bool m_logStart;
  uint32_t m_nodeId;
  uint32_t m_sndL;
  uint32_t m_rcvL;
