...
#ifndef MP_RDMA_HEADER_H
#define MP_RDMA_HEADER_H
...
/**
 * \ingroup mp-rdma
 * \brief Header for the Multi-Path RDMA 
 *
 * This class has fields corresponding to those in a network MP-RDMA header 
...
class MpRDMAHeader : public Header
{
public:
  MpRDMAHeader ();
  virtual ~MpRDMAHeader (); 

  /**
   * \brief Print a MpRDMA header into an output stream
   *
   * \param os output stream
   * \param tc MpRDMA header to print
   * \return The ostream passed as first argument
   */
  friend std::ostream& operator<< (std::ostream& os, MpRDMAHeader const & tc); 

  /**
   * \brief Converts an integer into a human readable list of MpRDMA flags
   *
   * \param flags Bitfield of MpRDMA flags to convert to a readable string
   * \param delimiter String to insert between flags
   *
   * FIN=0x1, SYN=0x2, RST=0x4, PSH=0x8, ACK=0x10, URG=0x20, ECE=0x40, CWR=0x80
   * MpRDMAHeader::FlagsToString (0x1) should return the following string;
   *     "FIN"
   *
   * MpRDMAHeader::FlagsToString (0xff) should return the following string;
  ...
  /**
   * \brief Enable checksum calculation for MpRDMA 
   *
 ...
 * \param port The source port for this MpRDMAHeader
 */
  void SetSourcePort (uint16_t port);

  /**
   * \brief Set the destination port
   * \param port the destination port for this MpRDMAHeader
   */
  void SetDestinationPort (uint16_t port);

  /**
   * \brief Set the sequence Number
   * \param sequenceNumber the sequence number for this MpRDMAHeader
   */
  void SetSequenceNumber (SequenceNumber32 sequenceNumber);

  /**
   * \brief Set the ACK number
   * \param ackNumber the ACK number for this MpRDMAHeader 
   */
  void SetAckNumber (SequenceNumber32 ackNumber);

  /**
   * \brief Set flags of the header
   * \param flags the flags for this MpRDMAHeader 
   */
  void SetFlags (uint8_t flags);

  /**
   * \brief Set the window size
   * \param windowSize the window size for this MpRDMAHeader 
   */
  void SetWindowSize (uint16_t windowSize);

  /**
   * \brief Set the urgent pointer
   * \param urgentPointer the urgent pointer for this MpRDMAHeader
   */
...
//Getters
   ...
   * \return The source port for this MpRDMAHeader
   */
  uint16_t GetSourcePort () const;

  /**
   * \brief Get the destination port
   * \return the destination port for this MpRDMAHeader
   */
  uint16_t GetDestinationPort () const;

  /**
   * \brief Get the sequence number
   * \return the sequence number for this MpRDMAHeader
   */
  SequenceNumber32 GetSequenceNumber () const;

  /**
   * \brief Get the ACK number
   * \return the ACK number for this MpRDMAHeader
   */
   ...
   *
   * \return the length of this MpRDMAHeader
   */
  uint8_t GetLength () const;

  /**
   * \brief Get the flags
   * \return the flags for this MpRDMAHeader
   */
  uint8_t GetFlags () const;

  /**
   * \brief Get the window size
   * \return the window size for this MpRDMAHeader
   */
  uint16_t GetWindowSize () const;

  /**
   * \brief Get the urgent pointer
   * \return the urgent pointer for this MpRDMAHeader
   */
   ...
   */
  /*
   * Mp-RDMA protocol header options follow the defination of TCP header options
   */ 
  Ptr<TcpOption> GetOption (uint8_t kind) const;

  /**
   * \brief Get the total length of appended options
   * \return the total length of options appended to this MpRDMAHeader
   */
  ...
  /**
   * \brief Append an option to the MpRDMA header 
  ...
  /**
   * \brief Initialize the MpRDMA checksum.
   *
   * If you want to use mp-rdma checksums, you should call this
  ...
  /**
   * \brief Initialize the MP-RDMA checksum.
   *
   * If you want to use mp-rdma checksums, you should call this
   * method prior to adding the header to a packet.
   *
  ...
  /**
   * \brief MP-RDMA flag field values 
   */
  typedef enum
  {
    NONE = 0,   //!< No flags
  ...
  /**
   * \brief Is the MP-RDMA checksum correct ? 
   ...
   */
  friend bool operator== (const MpRDMAHeader &lhs, const MpRDMAHeader &rhs);
...
private:
  ...
  typedef std::list< Ptr<TcpOption> > MpRDMAOptionList; //!< List of TcpOption 
  MpRDMAOptionList m_options;     //!< TcpOption present in the header
  uint8_t m_optionsLen;        //!< MpRDMA options length. 
};
