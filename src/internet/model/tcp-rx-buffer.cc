...
TcpRxBuffer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpRxBuffer")
    ...
    .AddAttribute ("StartLogOOO", "Signal for Log Start OOO",
                    BooleanValue (false),
                    MakeBooleanAccessor (&TcpRxBuffer::m_logStart),
                    MakeBooleanChecker ())
...
{
    m_maxSeqInBuf = SequenceNumber32(0);
    m_logStart = true;
}
...
bool
TcpRxBuffer::Add (Ptr<Packet> p, MpRDMAHeader const& mpRDMAh)
{
  NS_LOG_FUNCTION (this << p << mpRDMAh);

  /* ==================== for out-of-order measurement =============================== */
  if(m_logStart)
  {
      m_maxSeqInBuf = SequenceNumber32(0);
      m_oooSize = 0;
      ...
      char file_name[1024];
      sprintf(file_name, "Node_%u_OOO_sndL_%u_rcvL_%u.txt", m_nodeId, m_sndL, m_rcvL);
      LogManager::RegisterLog(28300 + m_nodeId, file_name);

      if(mpRDMAh.GetDestinationPort() <= 1008)
      {
        RecordOOO();
      }
      m_logStart = false;
  ...
  //printf("node %u received data %u\n", m_nodeId, mpRDMAh.GetSequenceNumber().GetValue());

  if(m_maxSeqInBuf < mpRDMAh.GetSequenceNumber())
  {
      m_maxSeqInBuf = mpRDMAh.GetSequenceNumber();
  }
  m_oooSize = m_maxSeqInBuf.GetValue() - m_nextRxSeq.Get().GetValue();
  if(mpRDMAh.GetDestinationPort() <= 1008)
  {
      RecordOOO();
  }
  /* ==================== End out-of-order measurement =============================== */

  uint32_t pktSize = p->GetSize ();
  SequenceNumber32 headSeq = mpRDMAh.GetSequenceNumber ();
  SequenceNumber32 tailSeq = headSeq + SequenceNumber32 (pktSize);
  NS_LOG_LOGIC ("Add pkt " << p << " len=" << pktSize << " seq=" << headSeq
                           << ", when NextRxSeq=" << m_nextRxSeq << ", buffsize=" << m_size);
  ...
  if (headSeq >= tailSeq)
    ...
    {
      uint32_t start = headSeq - mpRDMAh.GetSequenceNumber ();
      uint32_t length = tailSeq - headSeq;
      p = p->CreateFragment (start, length);
      NS_ASSERT (length == p->GetSize ());
    }
...
/* ==================== for out-of-order measurement =============================== */
void
TcpRxBuffer::RecordOOO ()
{
    LogManager::WriteLog(28300 + m_nodeId, "%u\n", m_oooSize);

    //printf("at node %u, OOO %u\n", m_nodeId, m_oooSize);
    
    //Simulator::Schedule(Time::FromDouble(0.0001, Time::S), &TcpRxBuffer::RecordOOO, this);
}
/* ==================== End out-of-order measurement =============================== */
