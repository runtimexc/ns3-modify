...
TcpSocketBase::TcpSocketBase (const TcpSocketBase& sock)
  : TcpSocket (sock),
    ...
    m_cwr_max(sock.m_cwr_max),
    m_setCWR(sock.m_setCWR) 
  ...
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
      ...
      if (!m_txBuffer->Add (p))
        { // TxBuffer overflow, send failed
          ...
          //printf("node %u, tx buffer overflow, size %u\n", GetNode()->GetId(), m_txBuffer->Size()); 
          return -1;
        }
      if (m_shutdownSend)
        {
          m_errno = ERROR_SHUTDOWN;
          printf("node %u, eror shut down \n", GetNode()->GetId()); 
          return -1;
        }
    ...
    { // Connection not established yet
      ...
      printf("node %u, connection not established \n", GetNode()->GetId()); 
                            ...
                            const Address &toAddress)
{
    ...
    if(m_ecn)
    {
        //decide the ECN state with incoming packets 
        //enable ECN 
        if(packet->PeekPacketTag(ecnTag))
        {
            //the packet has an ECN tag 
            if(ecnTag.ip_bit_1 && ecnTag.ip_bit_2)
            {
                //if CE flag is set in IP header 
                //change ECN state to CE_STATE 
                m_ecnState = CE_STATE; 
            }
            if(ecnTag.tcp_bit_1)
            {
                //if CWR flag is set in TCP header 
                //change ECN state to CE_CLEAR_STATE 
                m_ecnState = CE_CLEAR_STATE; 
            }
        }
    }
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if(m_ecn)
  {
      ...
      {
          //the packet has an ECN tag
          if(ecnTag.tcp_bit_2 == true && tcpHeader.GetAckNumber().GetValue() > m_cwr_max)
          {
              // ECE bit set, needs to react 
              HandleECE(); //for cwnd and ssthresh 
              m_setCWR = true; //set next new data packet with CWR 
              m_cwr_max = m_highTxMark.Get().GetValue(); 
          }
      }
  }
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if(isAck)
  {
      if(m_ecn && m_ecnState == CE_STATE)
      {
          //Set the ECE bit in the ACK 
          EcnTag ecnTag; 
          ecnTag.tcp_bit_2 = true; 
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if(m_ecn)
  {
      //attach an ECN marked tag 
      EcnTag ecnTag; 
      ecnTag.ip_bit_1 = true; //ECT(0) 
      //if m_cwr_max is set, send ECN tag with CWR and change m_setCWR 
      if(m_setCWR)
      {
          ecnTag.tcp_bit_1 = true; 
          m_setCWR = false; 
      }
      p->AddPacketTag(ecnTag); 
  }
        ...
        { // Ok to use this sample
          if (m_timestampEnabled && tcpHeader.HasOption (TcpOption::TS))
          ...
          else
            {
              ...
              //printf("now time is %lf, send time is %lf\n", Simulator::Now().GetSeconds(), h.time.GetSeconds()); 
            }
        }
    }
  ...
  if (!m.IsZero ())
    {
      ...
      //printf("RTT sample is %lf, RTO is %lf\n", m.GetSeconds(), m_rto.Get().GetSeconds()); 
    }
}
...
void
TcpSocketBase::HandleECE()
{
    m_tcb->m_ssThresh = std::max(2 * m_tcb->m_segmentSize, BytesInFlight () / 2); 
    m_tcb->m_cWnd = m_tcb->m_ssThresh + 3 * m_tcb->m_segmentSize; 
